...
function init()
	...
	["player"] ={ 255, 216, 0,255},
	["monster"] ={255, 0, 0, 255},
	["npc"] = {174, 0, 255, 255}
	}
	self.modNames = {
	["ageisalt"] = "Aegisalt Ore",
...
function drawCanvas(doLabel)
	if self.polyList ~= nil then
		...
		if self.modPositions ~= {} then
			for i,j in ipairs(self.modPositions) do
				console.canvasDrawRect({j[1], j[2], j[1]+1, j[2]+1}, self.colorScheme[j[3]] or {150,255,150,255})
			end
		end
		...
		if self.entityPositions ~= {} then
			for i,j in pairs(self.entityPositions) do
				console.canvasDrawPoly({vec2.add(j[2], {-2, -4}),vec2.add(j[2], {0, 2}),vec2.add(j[2], {2, -4})}, self.colorScheme[j[1]],0.5)
			end
		end
	else
...
function drawTip()
	console.canvasDrawRect({1, 189, 122, 210}, {230,230,230,255})
	console.canvasDrawRect({3, 191, 120, 208}, {0,0,0,255})
	console.canvasDrawText(self.modNames[self.toolTip[1]] or "Unknown", {position = {8, 204}, centered = true}, 10, self.colorScheme[self.toolTip[1]] or {255,255,255,255})
end
...
function craftPoly(x, y, iType)
	...
	local direction = traceEdge(pos, self.scanMatrix[pos[1]][pos[2]][1], nil)
	...
	if direction ~= nil then
		while timeout < 20000 do
			...
			newDirection = traceEdge(pos, self.scanMatrix[pos[1]][pos[2]][1], direction)
...
function traceEdge(pos, iType, inVec)
	local left = true
	local up = true
	local right = true
	local down = true
	if pos[1] > 1 then
		left = tryBoundary(iType, self.scanMatrix[pos[1]-1][pos[2]][1])
	end
	if pos[2] < adjustedBounds[2] then
		up = tryBoundary(iType, self.scanMatrix[pos[1]][pos[2]+1][1])
	end
	if pos[1] < adjustedBounds[1] then
		right = tryBoundary(iType, self.scanMatrix[pos[1]+1][pos[2]][1])
	end
	if pos[2] > 1 then
		down = tryBoundary(iType, self.scanMatrix[pos[1]][pos[2]-1][1])
	end
	if inVec == nil then 
